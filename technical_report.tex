...
\begin{itemize}
  ...
  \item \texttt{results/metrics.json} (merged): Adds \texttt{boundary\_energy}, \texttt{background\_laplacian}, and \texttt{object\_ssim} aggregates so that downstream scripts (notably \texttt{eval\_metrics.py}) can compute $Q_{\text{agg}}$ with real boundary values.
\end{itemize}
