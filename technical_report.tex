...
\begin{enumerate}
  \item \textbf{Module-level model instantiation}: \texttt{process\_image.py} instantiates \texttt{ImageProcessor} and \texttt{ImageDisplayer} at import time (lines 370--371). This was chosen for interactive notebook usage but causes side effects when importing the module. The ablation script (\texttt{generate\_stylization\_ablation.py}) reimplements the classes to avoid this.
