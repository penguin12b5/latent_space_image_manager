...
- **VAE**: Stable Diffusion v1.4 AutoencoderKL loaded from local `./models` directory via the `diffusers` library (uses safetensors format on Windows, pickle `.bin` on other platforms). Provides `encode()` and `decode()` methods that map between pixel space $\mathbb{R}^{H \times W \times 3}$ and latent space $\mathbb{R}^{4 \times h \times w}$ where $(h, w) = (H/8, W/8)$.
...
#### 2.2.1 `process_image_dod_fade()` — DOD-FADE
...
2. **Subject extraction**: Faster R-CNN detects objects; each ROI is cropped from the original image (original pixels, not re-encoded).
3. **Compositing**: Each original subject crop is pasted onto the smoothed background using a fade mask (8% of minimum ROI dimension as the fade radius, power exponent 1.5). The PIL `Image.paste()` method performs the alpha compositing.

#### 2.2.2 `process_image_dod_sam()` — DOD-SAM

Identical to DOD-FADE except the mask is generated by SAM rather than the heuristic fade function. The original subject pixels (not re-encoded) are pasted onto the smoothed background using the SAM mask. SAM is invoked with the bounding box as a prompt (`multimask_output=False`). Falls back to fade mask if `SAM_CHECKPOINT` environment variable is unset or SAM inference fails.

#### 2.2.3 `process_image_lol()` — LOL-FADE and LOL-SAM

Implements latent-space blending via the `mask_type` parameter:

1. **Latent background**: Input is downscaled by 0.25, encoded to latent, then the latent is upscaled back to full latent resolution using `F.interpolate` with `scale_factor=1/scale` (bilinear mode).
...
The script evaluates all four methods across the 11 benchmark images (`car1-3`, `cat1-3`, `dog1-2`, `horse1-3`). Method results are read from `images/output/<method>/<image>_0.25.png`. It prints per-image scores and identifies the winner for each image.
...
Evaluates the four primary methods plus a raw `output` baseline (VAE reconstruction without method-specific blending). Uses a ResNet-50 feature extractor (avgpool features, 2048-dimensional) rather than the standard Inception-v3 network. Feature vectors are collected across all images per method, and FID is computed as:
...
Preferentially uses the `lpips` package with VGG backbone when available (images resized to 256x256, normalized to $[-1, 1]$). Falls back to a VGG-16 perceptual distance (MSE in feature space at layer 16, images resized to 224x224) if the package is not installed.
...
1. Runs Faster R-CNN (on CPU) to obtain the top-confidence bounding box.
2. Generates a **SAM mask** by running SAM ViT-H inference (hardcoded) with the bounding box as prompt.
...
It also regenerates the LaTeX table snippet (`results/quant_table.tex`) with the updated values.
...
Imports the metric functions directly from `evaluate_image_blending.py` and reads method results from `images_eval/<method>/`. Re-computes composite scores under each weighting scheme. For each scheme, reports:
...
1. **Module-level model instantiation**: `process_image.py` instantiates `ImageProcessor` and `ImageDisplayer` at import time (lines 519--520). This was chosen for interactive notebook usage but causes side effects when importing the module (e.g., `generate_stylization_ablation.py` reimplements the classes to avoid this).
...
|----------------|---------------|
| Eq. 1: $z = E(I)$ | `ImageProcessor.encode()` (line 82) |
| Eq. 2: $I' = D(z)$ | `ImageProcessor.decode()` (line 93) |
| Eq. 5: $d_{\text{edge}}$ | `get_fade_mask()` (lines 140--154) |
| Eq. 6: $M_{\text{FADE}}$ | `get_fade_mask()` (lines 152--157) |
| Eq. 7: Alpha compositing | `process_image_dod_fade()` (line 235, `PIL.paste`) |
| Eq. 8: SAM mask generation | `get_sam_mask()` (lines 176--203) |
| Eq. 10-12: Latent background prep | `process_image_lol()` (lines 353--360) |
| Eq. 14: Latent masking | `paste_latend_with_fade()` (lines 294--306) |
| Eq. 15: Latent integration | `paste_latend_with_sam()` (lines 308--332) |
| Eq. 16: Final decode | `process_image_lol()` (line 393) |
| $Q_{\text{img}}$ composite | `evaluate_method()` in `evaluate_image_blending.py` (lines 167--171) |
